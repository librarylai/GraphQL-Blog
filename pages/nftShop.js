...
export default function nftShop() {
  ...
  async function handleMintWoodNFT() {
    ...
    const receipt = await woodContract.methods.mint(walletAccount[0], 1).send({ from: walletAccount[0], value: amountToSend })
    if (receipt.status) {
    ...
    <div>
      ...
      <div>
        ...
        <ul>
          <li>
            WoodNFT：
            <Button style={{ textTransform: 'none' }} variant='outlined' color='primary' onClick={handleMintWoodNFT}>
              Mint WoodNFT
            </Button>
          </li>
        </ul>
